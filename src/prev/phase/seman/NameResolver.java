...
public class NameResolver extends AstFullVisitor<Object, NameResolver.Mode> {
	...
	public Object visit(AstTrees<?> trees, Mode arg) {
		...
		for(AstTree astTree : trees) {
			if(astTree instanceof AstTypeDecl){
				AstTypeDecl decl = (AstTypeDecl) astTree;
		...
		for(AstTree astTree : trees) {
			if(astTree instanceof AstVarDecl){
				AstVarDecl decl = (AstVarDecl) astTree;
				decl.accept(this, Mode.HEAD);
			}
		}
		...
		for(AstTree astTree : trees) {
			if(astTree instanceof AstFunDecl){
				AstFunDecl decl = (AstFunDecl) astTree;
				decl.accept(this, Mode.HEAD);
			}
		}
	...
	public Object visit(AstStmtExpr stmtExpr, Mode mode) {
		AstTrees <AstStmt> statements = stmtExpr.stmts();
